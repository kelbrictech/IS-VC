...
export to WebM or GIF — all client-side, no server, no build step.
...
- One external dependency loaded from a CDN in `index.html`: `gif.js`
  (used only for GIF export)
...
- **Export presets**: TikTok, Instagram Reels/Story, Instagram feed,
  ...
  custom size — output as WebM (video + audio) or GIF (no audio)

## Known limitations

- **No MP4 export.** Reliable in-browser MP4 encoding needs a much heavier
  dependency (ffmpeg.wasm, ~25MB on first load) that isn't wired in here.
  Export WebM and convert externally if you need MP4, or treat adding
  ffmpeg.wasm as a separate follow-up task.
- **No project save/reload.** Closing the tab loses the current sequence —
  there's no save-to-file or reopen flow yet.
